      ...
      <v-col cols='9' sm='8' md='3' lg='3'>
        <v-card class='elevation-12'>
          <v-toolbar color='primary' dark flat>
            <v-toolbar-title class='disable-select'>{{ $t('outlay') }}</v-toolbar-title>
            <v-spacer></v-spacer>
          ...
          <v-dialog v-model='deleteDialog' max-width='600px' persistent>
            <v-card class='elevation-12'>
              <v-card-title class="text-center">{{ $t('outlayPageDeleteItemText') }}</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
...
import {createOutlay, deleteOutlay, getOutlays, updateOutlay} from "@/api/outlay";
...
export default defineComponent({
  ...
  methods: {
    async loadFromServer() {
      this.items = [];
      this.loading = true;
      const response = await getOutlays();
    ...
    async onClickEditButton() {
      ...
      await updateOutlay(itemId, data)
        .then(() => {
          this.editDialog = false;
          this.selectItem = '';
          this.count = 0;
    ...
    async loadCategories() {
      const response = await getOutlayCategories();
